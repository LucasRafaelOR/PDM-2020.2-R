...
class CorAdapter (private val context: Context, private val dataSource: ArrayList<Cor>) : BaseAdapter() {
    ...
    {
        val rowView = inflater.inflate(R.layout.color_row_item, parent, false);
        val currCor = getItem(position)

        val name = rowView.findViewById<TextView>(R.id.colorName)
        val code = rowView.findViewById<TextView>(R.id.colorCode)
        val icon = rowView.findViewById<ImageView>(R.id.colorIcon)
